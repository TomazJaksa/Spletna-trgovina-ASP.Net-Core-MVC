        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (ModelState.IsValid)
            {
                ...
                */
                ...
                var naslovnik = await _userManager.FindByEmailAsync(Input.Email);

                var naslovnikIme = naslovnik.Ime + " " + naslovnik.Priimek;

                await _mailService.SendEmailAsync(sendGridIme, sendGridEmail, Input.Email, naslovnikIme,"Resetiranje gesla",$"{HtmlEncoder.Default.Encode(callbackUrl)}");
