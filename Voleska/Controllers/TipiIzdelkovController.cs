...
namespace Voleska.Controllers
{
    public class TipiIzdelkovController : Controller
    {
        ...
        public async Task<IActionResult> Edit(int id, [Bind("ID,Ime, Slika")] TipIzdelkaViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (imeSlike != null)
                    {
                        DeletePreviousImage(tipIzdelka.Slika);
                        tipIzdelka.Slika = imeSlike;
                    }
        ...
        public async Task<IActionResult> Preveri(string ime, string tip)
        {
            if (tip == null)
            {
                var tipiIzdelkov = from m in _context.TipiIzdelkov
                                   select m;


                tipiIzdelkov = tipiIzdelkov.Where(s => s.Ime == ime);
                ...
                tipiIzdelkov = tipiIzdelkov.Where(s => s.Ime == ime && s.Ime != tip);
